...
const { mockFileFindUnique, mockCanvasSnapshotUpsert, mockCanvasSnapshotFindUnique, mockDocumentSnapshotUpsert, mockDocumentSnapshotFindUnique, } = vi.hoisted(() => ({
...
vi.mock("../db.js", () => ({
    db: {
        ...
        canvasSnapshot: {
            upsert: (...args) => mockCanvasSnapshotUpsert(...args),
            findUnique: (...args) => mockCanvasSnapshotFindUnique(...args),
        },
        documentSnapshot: {
            upsert: (...args) => mockDocumentSnapshotUpsert(...args),
            findUnique: (...args) => mockDocumentSnapshotFindUnique(...args),
        },
    },
}));
...
describe("persistence", () => {
    ...
    describe("onLoadDocument (seeding, AC-11)", () => {
        ...
        it("seeds document from DocumentSnapshot.yjsUpdate binary", async () => {
            ...
            expect(targetFragment.toString()).toBe("<paragraph>Hello LiveFlows</paragraph>");
        });
    });
    describe("onStoreDocument (persistence, AC-10)", () => {
        it("persists canvas elements and appState to CanvasSnapshot", async () => {
            ...
            expect(mockCanvasSnapshotUpsert).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it("persists document with lossless yjsUpdate and content to DocumentSnapshot", async () => {
            ...
            vi.useRealTimers();
            await new Promise((r) => setTimeout(r, 10));
            expect(mockDocumentSnapshotUpsert).toHaveBeenCalledWith(expect.objectContaining({
                where: { fileId: "doc_1" },
                create: expect.objectContaining({
                    fileId: "doc_1",
