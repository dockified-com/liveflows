...
const { mockVerifyToken, mockFileFindUnique, mockWorkspaceMemberFindFirst } = vi.hoisted(() => ({
...
vi.mock("../db.js", () => ({
    db: {
        file: { findUnique: (...args) => mockFileFindUnique(...args) },
        workspaceMember: {
            findFirst: (...args) => mockWorkspaceMemberFindFirst(...args),
        },
    },
}));
...
describe("onAuthenticate", () => {
    ...
    it("rejects when token is invalid or verification throws (AC-4)", async () => {
        ...
        await expect(onAuthenticate({ token: "invalid_token", documentName: "file_123" })).rejects.toThrow("Unauthorized: invalid token");
    });
    it("rejects when token claims do not have a sub (AC-4)", async () => {
        mockVerifyToken.mockResolvedValue({});
        await expect(onAuthenticate({ token: "token_no_sub", documentName: "file_123" })).rejects.toThrow("Unauthorized: no sub claim");
    });
    it("rejects when file does not exist", async () => {
        mockVerifyToken.mockResolvedValue({ sub: "user_123" });
        mockFileFindUnique.mockResolvedValue(null);
        await expect(onAuthenticate({ token: "valid_token", documentName: "file_missing" })).rejects.toThrow("Forbidden: not a workspace member");
    });
    it("rejects when user is not a member of the file's workspace (AC-5)", async () => {
        ...
        await expect(onAuthenticate({ token: "valid_token", documentName: "file_123" })).rejects.toThrow("Forbidden: not a workspace member");
    });
