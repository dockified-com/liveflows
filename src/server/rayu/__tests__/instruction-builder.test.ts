...
describe("instruction-builder.ts", () => {
        ...
        fc.property(
          ...
          (instruction, fieldToEmpty) => {
            ...
            const result = validateInstruction(
              badInstruction as TaskInstruction,
            );
  ...
  describe("Property 3: Permitted files count is bounded", () => {
    it("should reject 0 and >50 permitted files", () => {
      fc.assert(
        fc.property(
          arbitraryInstruction,
          fc.array(nonEmptyString, { minLength: 51 }),
          (instruction, tooMany) => {
            const resultEmpty = validateInstruction({
              ...instruction,
              permittedFiles: [],
            } as TaskInstruction);
            ...
            const resultTooMany = validateInstruction({
              ...instruction,
              permittedFiles: tooMany,
            } as TaskInstruction);
            expect(resultTooMany.valid).toBe(false);
            expect(resultTooMany.error).toContain(
              "permittedFiles count is out of bounds",
            );
          },
        ),
      );
        ...
        fc.property(
          ...
          (
            ...
            const agentsStr = output.includes("## Project Conventions")
              ? output.slice(
                  output.indexOf("## Project Conventions"),
                  output.indexOf("## Task Description"),
                )
              : "";
            ...
            const dirStr = dirIndex !== -1 ? output.slice(dirIndex) : "";
            ...
            const injectedBytes =
              Buffer.byteLength(agentsStr, "utf8") +
  ...
  describe("Unit tests", () => {
    ...
    it("includes AGENTS.md when non-null and omits when null", () => {
      ...
      expect(outWithAgents).toContain(
        "## Project Conventions\nSome conventions\n\n",
      );

      const outWithoutAgents = buildInstructionText({
        ...baseCtx,
        agentsMdContent: null,
      });
