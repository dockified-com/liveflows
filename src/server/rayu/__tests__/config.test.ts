...
describe("Configuration module", () => {
  ...
  it("returns invalid JSON error", async () => {
    ...
    const result = await loadConfig("/dummy");
    expect(result).toEqual({ ok: false, error: "Invalid JSON in rayu-config.json" });
  });
  ...
  describe("Feature: rayu-worker-agent, Property 8: Configuration validation rejects out-of-range values and accepts valid ones", () => {
    const validConfigArbitrary = fc.record({
      ...
      cliFlags: fc.array(fc.string({ minLength: 1 }).filter(s => s.trim() !== ""), { maxLength: 20 }),
    ...
    const invalidTimeoutArb = fc.oneof(fc.integer({ max: 29 }), fc.integer({ min: 3601 }));
    it("rejects out-of-range timeoutSeconds", async () => {
      await fc.assert(
        fc.asyncProperty(validConfigArbitrary, invalidTimeoutArb, async (configObj, invalidTimeout) => {
          vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify({ ...configObj, timeoutSeconds: invalidTimeout }));
          const result = await loadConfig("/dummy");
          expect(result).toEqual({ ok: false, error: "timeoutSeconds must be between 30 and 3600" });
        })
      );
    });

    const invalidOutputArb = fc.oneof(fc.integer({ max: 1023 }), fc.integer({ min: 10485761 }));
    it("rejects out-of-range maxOutputBytes", async () => {
      await fc.assert(
        fc.asyncProperty(validConfigArbitrary, invalidOutputArb, async (configObj, invalidOutput) => {
          vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify({ ...configObj, maxOutputBytes: invalidOutput }));
          const result = await loadConfig("/dummy");
          expect(result).toEqual({ ok: false, error: "maxOutputBytes must be between 1024 and 10485760" });
        })
      );
    });
    ...
    it("rejects empty binaryPath", async () => {
      await fc.assert(
        fc.asyncProperty(validConfigArbitrary, fc.constant("   "), async (configObj, invalidBinaryPath) => {
          vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify({ ...configObj, binaryPath: invalidBinaryPath }));
          const result = await loadConfig("/dummy");
          expect(result).toEqual({ ok: false, error: "binaryPath must be a non-empty string" });
        })
      );
    });

    const oversizedFlagsArb = fc.array(fc.string({ minLength: 1 }).filter(s => s.trim() !== ""), { minLength: 21 });
    it("rejects oversized cliFlags", async () => {
      await fc.assert(
        fc.asyncProperty(validConfigArbitrary, oversizedFlagsArb, async (configObj, invalidFlags) => {
          vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify({ ...configObj, cliFlags: invalidFlags }));
          const result = await loadConfig("/dummy");
          expect(result).toEqual({ ok: false, error: "cliFlags must be an array with max 20 items" });
        })
      );
    });
    ...
    it("rejects empty strings in cliFlags", async () => {
      await fc.assert(
        fc.asyncProperty(validConfigArbitrary, async (configObj) => {
          vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify({ ...configObj, cliFlags: ["valid", "   "] }));
          const result = await loadConfig("/dummy");
          expect(result).toEqual({ ok: false, error: "cliFlags entries must be non-empty strings" });
        })
      );
    });
  ...
  describe("Feature: rayu-worker-agent, Property 9: Environment variable overrides take precedence over config file values", () => {
    const validConfigArbitrary = fc.record({
      ...
      timeoutSeconds: fc.integer({ min: 30, max: 3600 }),
      maxOutputBytes: fc.integer({ min: 1024, max: 10485760 }),
      cliFlags: fc.array(fc.string({ minLength: 1 }).filter(s => s.trim() !== ""), { maxLength: 20, minLength: 1 }), // Needs minLength 1 to format as string array
      maxContextBytes: fc.integer({ min: 1024, max: 1048576 }),
    });

    const validEnvArbitrary = fc.record({
      RAYU_BINARY_PATH: fc.string({ minLength: 1 }).filter(s => s.trim() !== ""),
      RAYU_TIMEOUT: fc.integer({ min: 30, max: 3600 }).map(String),
      RAYU_MAX_OUTPUT: fc.integer({ min: 1024, max: 10485760 }).map(String),
      RAYU_CLI_FLAGS: fc.array(fc.string({ minLength: 1 }).filter(s => s.trim() !== "" && !s.includes(",")), { maxLength: 20, minLength: 1 }).map(arr => arr.join(",")),
    }, { withDeletedKeys: true });
