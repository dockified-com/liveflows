...
export async function loadConfig(workspaceDir: string): Promise<ConfigResult> {
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        return {
          ok: false,
          error: "rayu-config.json must contain a JSON object",
        };
      }
    } catch (e) {
  ...
  if (process.env.RAYU_CLI_FLAGS !== undefined) {
    fileConfig.cliFlags = process.env.RAYU_CLI_FLAGS
      ? process.env.RAYU_CLI_FLAGS.split(",")
      : [];
  }
  ...
  const config: RayuConfig = {
    ...
    maxContextBytes:
      fileConfig.maxContextBytes ?? DEFAULT_CONFIG.maxContextBytes,
  };
  ...
  if (
    !Number.isInteger(config.maxOutputBytes) ||
    config.maxOutputBytes < 1024 ||
    config.maxOutputBytes > 10485760
  ) {
    return {
      ok: false,
  ...
  if (
    ...
    config.maxContextBytes > 1048576
  ) {
    return {
      ok: false,
