...
export function validateInstruction(instruction: TaskInstruction): {
  valid: boolean;
  error?: string;
} {
  if (!instruction.description || instruction.description.trim() === "") {
    return {
      valid: false,
      error: "Missing or empty mandatory field: description",
    };
  }
  if (
    !instruction.contextFiles ||
    instruction.contextFiles.length === 0 ||
    instruction.contextFiles.some((f) => !f || f.trim() === "")
  ) {
    return {
      valid: false,
      error: "Missing, empty, or invalid mandatory field: contextFiles",
    };
  }
  if (
    !instruction.constraints ||
    instruction.constraints.length === 0 ||
    instruction.constraints.some((c) => !c || c.trim() === "")
  ) {
    return {
      valid: false,
      error: "Missing, empty, or invalid mandatory field: constraints",
    };
  }
  if (
    !instruction.permittedFiles ||
    instruction.permittedFiles.length === 0 ||
    instruction.permittedFiles.some((f) => !f || f.trim() === "")
  ) {
    return {
      valid: false,
      error: "Missing, empty, or invalid mandatory field: permittedFiles",
    };
  }
  if (instruction.permittedFiles.length > 50) {
    return {
      valid: false,
      error: "permittedFiles count is out of bounds (1-50)",
    };
  }
  if (!instruction.expectedOutput || instruction.expectedOutput.trim() === "") {
    return {
      valid: false,
      error: "Missing or empty mandatory field: expectedOutput",
    };
  }
  return { valid: true };
}

const CREATION_TASK_REGEX =
  /\b(create|new|generate|build|add|make|scaffold)\b/i;
