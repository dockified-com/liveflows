...
test.describe("Excalidraw + React Compiler spike", () => {
  ...
  test("2. Drag a shape and assert bound arrow endpoint moved", async ({
    ...
    await page.mouse.click(
      (await page.locator(".excalidraw__canvas.interactive").boundingBox())!.x +
        150,
      (await page.locator(".excalidraw__canvas.interactive").boundingBox())!.y +
        140,
    );
  ...
  test("5. Change stroke colour on a selected shape", async ({ page }) => {
    ...
    const rectBefore = beforeElements.find(
    ...
    const colorPicker = page.locator('[data-testid="color-top-picks"] button');
